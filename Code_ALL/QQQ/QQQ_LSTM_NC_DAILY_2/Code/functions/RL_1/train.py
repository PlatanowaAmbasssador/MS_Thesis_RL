...
# =============================================================================
# HP CONFIGS — FINAL v2: Beat QQQ on return AND risk
# =============================================================================
# v1 beat QQQ on ARC (19.7% vs 19.1%) but MDD was too high (41% vs 36%).
# Fix: drop k10 (too concentrated → 41% MDD), add mild penalty (0.1)
# to limit extreme concentration while still allowing mega-cap bets.
...
DEFAULT_HP_CONFIGS = [
    # A) k20, mild penalty (0.1) — limits extreme concentration
    {"name": "bmk_mild_k20",
     "lstm_hidden": 64, "lstm_layers": 1, "dropout": 0.0,
     "lr_actor": 3e-4, "batch_size": 128, "top_k": 20,
     "variance_penalty": 0.1,
     **{k: v for k, v in _SHARED_FINAL.items() if k != "variance_penalty"}},

    # B) k20, no penalty (same as v1 best)
    {"name": "bmk_nopen_k20",
     "lstm_hidden": 64, "lstm_layers": 1, "dropout": 0.0,
     "lr_actor": 3e-4, "batch_size": 128, "top_k": 20,
     **_SHARED_FINAL},

    # C) Medium LSTM, k20, mild penalty
    {"name": "bmk_mild_med_k20",
