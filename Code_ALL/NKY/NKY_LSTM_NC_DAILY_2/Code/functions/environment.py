"""
environment.py — RL Environment for Portfolio Allocation (v5 — Daily Long-Short)
==================================================================================
v5 CHANGES over v4:
    - NEW reward_type="benchmark_relative": log(1+r_RL) - log(1+r_QQQ)
      Directly optimizes for beating the benchmark.
    - Supports NEGATIVE stock weights (short positions)
    - Higher TC for shorts: tc_short_multiplier (default 5x → 10bps)
    - Updated drift normalization for long-short portfolios
    - Updated exiting logic for short positions
"""
...
class PortfolioEnv:
    def __init__(self, dataset, start_date=None, end_date=None,
                 ...
                 allow_short=False, tc_short_multiplier=5.0,
                 force_fully_invested=False):
        ...
        self.tc_short_multiplier = tc_short_multiplier
        self.force_fully_invested = force_fully_invested
    ...
    def _action_to_weights(self, action, selected):
        n_selected = selected.sum()
        if self.allow_short:
            # 130/30 construction from Dirichlet weights:
            # 1. Raw Dirichlet gives positive weights summing to ~1
            # 2. Center: w_centered = w - mean → some become negative
            # 3. Scale: longs sum to 1.3, shorts sum to -0.3, net = 1.0
            raw_w = np.clip(action[:n_selected], 0, 1)
            cash_raw = np.clip(action[n_selected], 0, 1)
            total_raw = raw_w.sum() + cash_raw
            if total_raw > 0:
                raw_w = raw_w / total_raw
                cash_raw = cash_raw / total_raw

            # Center stock weights around mean → bottom stocks go negative
            mean_w = raw_w.mean() if len(raw_w) > 0 else 0
            centered = raw_w - mean_w  # some positive (long), some negative (short)

            # Scale to 130/30
            pos_sum = centered[centered > 0].sum()
            neg_sum = abs(centered[centered < 0].sum())
            if pos_sum > 1e-8 and neg_sum > 1e-8:
                stock_w = np.where(centered > 0,
                                   centered / pos_sum * 1.3,
                                   centered / neg_sum * (-0.3))
            else:
                stock_w = raw_w  # fallback to long-only

            cash_w = 0.0  # 130/30 is fully invested
            ...
            if total > 0:
                stock_w /= total
                cash_w /= total
            # Force 100% invested: redistribute cash pro-rata to stocks
            if self.force_fully_invested and cash_w > 1e-6:
                stock_sum = stock_w.sum()
                if stock_sum > 1e-8:
                    stock_w = stock_w / stock_sum  # renormalize to sum=1
                cash_w = 0.0
            full_w = np.zeros(self.n_tickers, dtype=np.float64)
            full_w[selected] = stock_w
            return full_w, float(cash_w)
    ...
    def step(self, action):
        ...
        # TC: higher for short trades
        if self.allow_short:
            long_turnover = np.abs(np.maximum(stock_w, 0) - np.maximum(old_stocks, 0)).sum()
            short_turnover = np.abs(np.minimum(stock_w, 0) - np.minimum(old_stocks, 0)).sum()
            cash_turnover = abs(cash_w - old_cash)
            tc = (long_turnover * self.tc_rate
                  + short_turnover * self.tc_rate * self.tc_short_multiplier
                  + cash_turnover * self.tc_rate)
            turnover = long_turnover + short_turnover + cash_turnover
        else:
        ...
        # Portfolio return: np.dot handles negative weights correctly
        ...
        # Drift: weights evolve with returns
        ...
        # Handle exiting stocks (both long AND short positions)
        tradable_t1 = self._get_tradable_mask(date_t1)
        exiting = (~tradable_t1) & (np.abs(self.weights[:self.n_tickers]) > 1e-8)
        if exiting.any():
            ew = self.weights[:self.n_tickers][exiting].sum()
            self.weights[:self.n_tickers][exiting] = 0.0
            # Redistribute to remaining positions (proportional)
            remaining = tradable_t1 & (np.abs(self.weights[:self.n_tickers]) > 1e-8)
            if remaining.any():
                rem_sum = self.weights[:self.n_tickers][remaining].sum()
                if abs(rem_sum) > 1e-8:
                    self.weights[:self.n_tickers][remaining] += ew * self.weights[:self.n_tickers][remaining] / rem_sum
        ...
        if self.reward_type == "benchmark_relative":
            # ========================================================
            # NEW: Excess return over QQQ benchmark
            # reward = [log(1+r_RL) - log(1+r_QQQ)] × 1000
            # Directly optimizes for beating the benchmark.
            # ========================================================
            port_log_ret = np.log1p(max(port_ret_net, -0.999))
            qqq_log_ret = np.log1p(max(float(qqq_ret), -0.999))
            excess = (port_log_ret - qqq_log_ret) * 1000.0
            ...
            reward = excess - to_penalty - conc_penalty
            ...
            reward = self.diff_sharpe.compute(port_ret_net) * 100.0
            reward -= self.turnover_penalty * turnover * 100.0
        else:
            reward = port_ret_net * 100.0
        ...
        info = {
            ...
            "tc": tc,
            "qqq_return": qqq_ret,
            "n_tradable": self._current_selected.sum() if self._current_selected is not None else 0,
        }
        return next_state, reward, self.done, info

    def get_results(self):
        idx = pd.DatetimeIndex(self.history["date"])
        return {
            "portfolio_return": pd.Series(self.history["portfolio_return"], index=idx),
            "portfolio_return_net": pd.Series(self.history["portfolio_return_net"], index=idx),
            "turnover": pd.Series(self.history["turnover"], index=idx),
            "transaction_cost": pd.Series(self.history["transaction_cost"], index=idx),
            "reward": pd.Series(self.history["reward"], index=idx),
            "portfolio_value": pd.Series(self.history["portfolio_value"], index=idx),
            "qqq_return": pd.Series(self.history["qqq_return"], index=idx),
            "cash_weight": pd.Series(self.history["cash_weight"], index=idx),
            "equity_fraction": pd.Series(self.history["equity_fraction"], index=idx),
            "rf_earned": pd.Series(self.history["rf_earned"], index=idx),
        }
