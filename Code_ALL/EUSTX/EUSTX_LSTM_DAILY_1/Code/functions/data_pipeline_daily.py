...
def clean_and_align(raw: dict) -> dict:
    ...
    # Some datasets may contain invalid/non-positive "close" values (e.g., negatives).
    # They break log-based features and can create unstable returns, so we treat them as missing.
    prices = prices.where(prices > 0)
    # If we just introduced NaNs, do a final short forward-fill so isolated bad prints don't kill a day.
    prices = prices.ffill(limit=1)
    # Ensure the membership mask only allows tradable assets when we have a valid positive price.
    mask = mask.where(prices.notna(), 0).astype(int)
...
def compute_rsi(close, period=14):
    ...
    # Guard log inputs: RSI uses log-returns, which are undefined for non-positive prices.
    close_pos = close.where(close > 0)
    log_ret = np.log(close_pos).diff()
...
def compute_rolling_beta(close, qqq_series, period=60):
    ...
    # Guard log inputs: beta uses log-returns of both the asset and QQQ.
    close_pos = close.where(close > 0)
    qqq_pos = qqq_series.where(qqq_series > 0)
    log_ret = np.log(close_pos).diff()
    qqq_ret = np.log(qqq_pos).diff()
...
def build_per_asset_features(clean):
    ...
    features_raw = {}
    # Guard log inputs: several features use log-price / log-returns.
    close_pos = close.where(close > 0)
...
def build_global_features(clean):
    ...
    # --- Market breadth ---
    ret_5d = np.log(close_pos).diff(5)
    ...
    # --- QQQ momentum ---
    qqq_pos = qqq.where(qqq > 0)
    qqq_log = np.log(qqq_pos)
