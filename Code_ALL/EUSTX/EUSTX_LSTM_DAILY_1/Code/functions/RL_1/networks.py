...
class StateProcessorV2(nn.Module):
    ...
    def forward(self, state_dict: dict):
        ...
        # When n_tradable == 1, the default unbiased std can become NaN (ddof=1).
        # Use unbiased=False so std is well-defined for single-element samples.
        w_std = stock_w.std(dim=1, keepdim=True, unbiased=False).clamp(min=1e-8)
...
class Critic(nn.Module):
    ...
    def _action_stats(self, weights, n_tradable):
        ...
        a_std = stock_w.std(dim=1, keepdim=True, unbiased=False).clamp(min=1e-8)
